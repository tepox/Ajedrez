...
require Logger
  test "inicia juego" , %{jugadores: {jugador1, jugador2}}  do
    ...
    assert Map.get(Map.get(ajedrez.tablero, :a), 1).nombre == :torre
    assert Map.get(Map.get(ajedrez.tablero, :b), 8).nombre == :caballo
    assert Map.get(Map.get(ajedrez.tablero, :c), 1).nombre == :alfil
    assert Map.get(Map.get(ajedrez.tablero, :d), 8).nombre == :dama
    assert Map.get(Map.get(ajedrez.tablero, :e), 1).nombre == :rey
    assert Map.get(Map.get(ajedrez.tablero, :f), 8).nombre == :alfil
    assert Map.get(Map.get(ajedrez.tablero, :g), 8).nombre == :caballo
    assert Map.get(Map.get(ajedrez.tablero, :h), 8).nombre == :torre
    assert Ajedrez.JugadorControl.get_rey(ajedrez.jugador1) == Map.get(Map.get(ajedrez.tablero, :e), 1)
    assert Ajedrez.JugadorControl.get_rey(ajedrez.jugador2) == Map.get(Map.get(ajedrez.tablero, :e), 8)
  end

  test "pieza tablero" do
    ajedrez = TableroControl.inicia_juego(%Tablero{}, %Jugador{}, %Jugador{})
    assert TableroControl.pieza_tablero(ajedrez.tablero, "a", 2).nombre == :peon
    assert TableroControl.pieza_tablero(ajedrez.tablero, "a", 7).nombre == :peon
    assert TableroControl.pieza_tablero(ajedrez.tablero, "a", 5) == nil
  end
